...
const passportConf = reqlib('config/passport.js');
...
/* 글쓰기 페이지를 보내준다. */
router.get('/write', passportConf.isAuthenticated, (req, res) => {
...
/* Post를 만든다. */
router.post('/', passportConf.isAuthenticated, (req, res) => {
...
/* post를 수정한다. */
router.patch('/:postId', passportConf.isAuthenticated, (req, res) => {
	const postId = parseInt(req.params.postId);
...
router.delete('/:postId', passportConf.isAuthenticated, (req, res) => {
	const postId = parseInt(req.params.postId);
