...
router.post('/add', (req, res) => {
	...
	.then( ( {count, id} ) => {
		const newPost = { _id: id, title: req.body.title, content: req.body.content };
...
/* Post를 count개 만든다. */
router.get('/add/:count', (req, res) => {
	let iterCount = req.params.count;
	let counterQuery = {name: 'postId'},
    	counterUpdate = { $inc: { count: iterCount, id: iterCount } },
 		counterOptions = { upsert: true };
	...
	.then( ( {count, id} ) => {
		...
		for(let i = 0; i < iterCount; i++)
			newPosts.push({ _id: id + i, title: "title" + (id + i), content: "content" + (id + i)});
		
		//{ _id: id, title: req.body.title, content: req.body.content };
